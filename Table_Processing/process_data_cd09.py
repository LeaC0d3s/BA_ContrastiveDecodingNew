...
table_a = pd.read_excel(xls, 'Blatt G - prob0.9_comp_table_G', header=2)
...
filtered_table_a = table_a[table_a['tok1_int_cd9']!= 13].dropna(subset=['tok1_int_ger'])
...
for name, group in grouped:
    ...
    if 'tok1_cd9%' in group.columns and not group['tok1_cd9%'].isna().all():
        tok1_cd5_percents = group['tok1_cd9%'].reset_index(drop=True)
        plt.plot(range(1, len(tok1_cd5_percents) + 1), tok1_cd5_percents, label='tok1_cd9%', marker='o', color='red', zorder=1)
    ...
    for i in range(len(group)):
        if pd.notna(group['tok1_ger%'].iloc[i]):
            ...
            if group['tok1_int_cd9'].iloc[i] == group['tok1_int_ger'].iloc[i]:
        ...
        if pd.notna(group['tok2_ger%'].iloc[i]):
            ...
            if group['tok1_int_cd9'].iloc[i] == group['tok2_int_ger'].iloc[i]:
        ...
        if pd.notna(group['tok3_ger%'].iloc[i]):
            ...
            if group['tok1_int_cd9'].iloc[i] == group['tok3_int_ger'].iloc[i]:
    ...
    for i in range(len(group)):
        if pd.notna(group['tok1_en%'].iloc[i]):
            ...
            if group['tok1_int_cd9'].iloc[i] == group['tok1_int_en'].iloc[i]:
        ...
        if pd.notna(group['tok2_en%'].iloc[i]):
            ...
            if group['tok1_int_cd9'].iloc[i] == group['tok2_int_en'].iloc[i]:
        ...
        if pd.notna(group['tok3_en%'].iloc[i]):
            ...
            if group['tok1_int_cd9'].iloc[i] == group['tok3_int_en'].iloc[i]:
    ...
    plt.title(f'Probabilities for CD: 0.9 - Idx {name}')
    plt.xlabel('Token')
    ...
    plt.grid(True)
    #plt.show()
    plot_file = os.path.join(output_dir, f'plot_0.9_{name}.png')
...
excel_file = 'Filter_CD09_G.xlsx'
workbook = xlsxwriter.Workbook(excel_file)
worksheet = workbook.add_worksheet('Filter CD09 G')
